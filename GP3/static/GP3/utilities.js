		...
		if (data.length == headers.length && i < 28) {
			...
			for (var j = 0; j < data.length; j++) {
				...
				if (j == 0) {
					...
					round4.push([curr.substring(0, curr.length - 2), curr.substring(curr.length - 2, curr.length - 1), curr.substring(curr.length - 1)]);
				}
			}
		}
	}
	EMlist = [round1, round2, round3, round4];
	// Sets trialData global to the matrix generated by the method
}

var soundDurations = {};

// Takes info from CSV file and obtains durations of all stimuli
function processTxt(allText) {
	...
	for (var i = 0; i < allTextLines.length; i++) {
		...
		// the CSV file is not properly formatted
		if (data.length == headers.length) {
			var soundName = data[0];
			var tot_dur = data[1];
			var w1_dur = data[2];
			var sil = data[3];
			var w2_dur = data[4];
			soundDurations[soundName] = {"tot_dur": tot_dur,"w1_dur": w1_dur,"sil": sil, "w2_dur": w2_dur}
...
function imageFileName(imageNumber, plural = false, gb = false, bigger = false) {
	...
	// Small images
	if (imageNumber < 12) {
		return grb + bggr + "l" + (imageNumber + 1) + plu + ".png";
	}

	// Large images
	else {
		return grb + bggr + "h" + (imageNumber - 12 + 1) + plu + ".png";
	}
}
...
function makeExp() {
	...
	var preloadTraining = {
    	...
    	audio: preload_sounds_training
	};

	console.log(preload_images)
	console.log(preload_sounds_training)
	...
	browser_id = platform.description;
	...
	var send_2AFC_data = {
	    type: 'call-function',
	    func: function(){send_data_to_server(total_data_array, "2AFC")}
	}
	experiments.push(send_2AFC_data)
	...
	var send_4AFC_data = {
	    type: 'call-function',
	    func: function(){send_data_to_server(total_data_array, "4AFC")}
	}
	experiments.push(send_4AFC_data)
	...
	var send_EM_data = {
	    type: 'call-function',
	    func: function(){send_data_to_server(total_data_array, "EM")}
	}
	experiments.push(send_EM_data)
