...
function production_test_trial(image1, sound, prompt,plurality,alienidentifiernr, neighborhood, subset) {
	...
	var active_production_trial = {
		...
		func: function() {
			...
			var data_array = [subjectnr, cond, trialnr, "PT", alienidentifiernr,sound, neighborhood, subset, plurality, image1, data_from_current_node.select('rt').values[0], "-", "-", "-", JSON.parse(data_from_current_node.select('responses').values[0])["first"], prompt,"-", "-","-", "-","-", "-"]
