...
function active_comprehension_trial(image1, image2, match, sound, prompt,plurality,alienidentifiernr) {
    ...
    console.log("audioFileName active comp trial", audioFileName)
	...
	let active_comprehension_trial = {
		...
		func: function() {
			...
			var data_array = [subjectnr, cond, trialnr, "AC", alienidentifiernr, sound, neighborhood, "training", plurality, image2, data_from_current_node.select('rt').values[0],soundDurations[audioFileName]["tot_dur"],soundDurations[audioFileName]["w1_dur"],soundDurations[audioFileName]["sil"],soundDurations[audioFileName]["w2_dur"], were_they_correct, data_from_current_node.select('response').values[0], cor_key, "-", "-", "-", match, image1, "-", "-", "-", "-", "-", "-", "-"]
